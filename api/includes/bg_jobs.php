...
function runJob_approveVisit($pdo, $payload) {
    ...
    } catch (Throwable $e) { error_log("[BG] PDF error: " . $e->getMessage()); }

    // 2. WhatsApp to visitor
    try {
        require_once dirname(__DIR__) . '/../includes/whatsapp_helper.php';
        sendWhatsAppNotification(
            $visit['mobile'], "Your visit is approved",
            'visit_approval_visitor_notify', ["*{$visit['visitor_name']}*"], $pdfUrl
        );
...
function runJob_rejectVisit($pdo, $payload) {
    ...
    try {
        require_once dirname(__DIR__) . '/../includes/whatsapp_helper.php';
        sendWhatsAppNotification(
            $visit['mobile'], "Your visit request has been declined.",
            'visit_rejection_visitor_notify', ["*{$visit['visitor_name']}*", "*{$reason}*"]
        );
...
function runJob_cancelInvite($pdo, $payload) {
    ...
    try {
        require_once dirname(__DIR__) . '/../includes/whatsapp_helper.php';
        sendWhatsAppNotification($visitor_mobile, "Your meeting has been cancelled.",
            'invite_cancelled', [$visitor_name, $host_name]
        );
    } catch (Throwable $e) { error_log("[BG] WhatsApp cancel error: " . $e->getMessage()); }
...
function bgHelper_generateQrCode($visit_code, $visit_id, $pdo) {
    ...
    } catch (Throwable $e) { error_log("[BG] QR error: " . $e->getMessage()); }
}
